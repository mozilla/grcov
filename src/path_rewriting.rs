...
pub fn rewrite_paths(result_map: CovResultMap, path_mapping: Option<Value>, source_dir: PathBuf, prefix_dir: &str, ignore_global: bool, ignore_not_existing: bool, to_ignore_dirs: Vec<String>, filter_option: Option<bool>) -> CovResultIter {
...
mod tests {
    ...
    fn test_rewrite_paths_basic() {
        ...
        let results = rewrite_paths(result_map, None, PathBuf::from(""), "", false, false, Vec::new(), None);
        let mut count = 0;
    ...
    fn test_rewrite_paths_ignore_global_files() {
            ...
            let results = rewrite_paths(result_map, None, PathBuf::from(""), "", true, false, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
                assert_eq!(abs_path, PathBuf::from("main.cpp"));
    ...
    fn test_rewrite_paths_remove_prefix() {
            ...
            let results = rewrite_paths(result_map, None, PathBuf::from(""), "/home/worker/src/workspace/", false, false, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
                assert_eq!(abs_path, PathBuf::from("main.cpp"));
    ...
    fn test_rewrite_paths_remove_prefix() {
            ...
            let results = rewrite_paths(result_map, None, PathBuf::from(""), "C:\\Users\\worker\\src\\workspace\\", false, false, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
                assert_eq!(abs_path, PathBuf::from("main.cpp"));
    ...
    fn test_rewrite_paths_remove_prefix_with_slash_longer_path() {
            let mut result_map: CovResultMap = HashMap::new();
            result_map.insert("C:/Users/worker/src/workspace/main.cpp".to_string(), empty_result!());
            let results = rewrite_paths(result_map, None, PathBuf::from(""), "C:/Users/worker/src/", false, false, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
    ...
    fn test_rewrite_paths_ignore_non_existing_files() {
            ...
            let results = rewrite_paths(result_map, None, PathBuf::from(""), "", false, true, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
    ...
    fn test_rewrite_paths_ignore_a_directory() {
            ...
            let results = rewrite_paths(result_map, None, PathBuf::from(""), "", false, false, vec!["mydir/*".to_string()], None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
                assert_eq!(abs_path, PathBuf::from("main.cpp"));
    ...
    fn test_rewrite_paths_ignore_multiple_directories() {
            ...
            let results = rewrite_paths(result_map, None, PathBuf::from(""), "", false, false, vec!["mydir/*".to_string(), "mydir2/*".to_string()], None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
                assert_eq!(abs_path, PathBuf::from("main.cpp"));
    ...
    fn test_rewrite_paths_rewrite_path_using_relative_source_directory() {
            ...
            let results = rewrite_paths(result_map, None, fs::canonicalize("tests").unwrap(), "", false, true, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
                assert!(abs_path.is_absolute());
    ...
    fn test_rewrite_paths_rewrite_path_and_remove_prefix() {
            ...
            let results = rewrite_paths(result_map, None, fs::canonicalize("tests").unwrap(), "/home/worker/src/workspace", false, true, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
                assert!(abs_path.is_absolute());
    ...
    fn test_rewrite_paths_rewrite_path_and_remove_prefix() {
            ...
            let results = rewrite_paths(result_map, None, fs::canonicalize("tests").unwrap(), "C:\\Users\\worker\\src\\workspace", false, true, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
                assert!(abs_path.is_absolute());
    ...
    fn test_rewrite_paths_rewrite_path_using_mapping() {
            let mut result_map: CovResultMap = HashMap::new();
            result_map.insert("class/main.cpp".to_string(), empty_result!());
            let results = rewrite_paths(result_map, Some(json!({"class/main.cpp": "rewritten/main.cpp"})), PathBuf::from(""), "", false, false, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
    ...
    fn test_rewrite_paths_rewrite_path_using_mapping_and_ignore_non_existing() {
            ...
            result_map.insert("tests/class/main.cpp".to_string(), empty_result!());
            let results = rewrite_paths(result_map, Some(json!({"rewritten/main.cpp": "tests/class/main.cpp", "tests/class/main.cpp": "rewritten/main.cpp"})), PathBuf::from(""), "", false, true, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
                assert!(abs_path.is_absolute());
    ...
    fn test_rewrite_paths_rewrite_path_using_mapping_and_remove_prefix() {
            ...
            let results = rewrite_paths(result_map, Some(json!({"/home/worker/src/workspace/rewritten/main.cpp": "tests/class/main.cpp"})), PathBuf::from(""), "/home/worker/src/workspace", false, true, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
                assert!(abs_path.is_absolute());
    ...
    fn test_rewrite_paths_rewrite_path_using_mapping_and_remove_prefix() {
            ...
            let results = rewrite_paths(result_map, Some(json!({"C:/Users/worker/src/workspace/rewritten/main.cpp": "tests/class/main.cpp"})), PathBuf::from(""), "C:\\Users\\worker\\src\\workspace", false, true, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
                assert!(abs_path.is_absolute());
    ...
    fn test_rewrite_paths_rewrite_path_using_mapping_and_source_directory_and_remove_prefix() {
            let mut result_map: CovResultMap = HashMap::new();
            result_map.insert("/home/worker/src/workspace/rewritten/main.cpp".to_string(), empty_result!());
            let results = rewrite_paths(result_map, Some(json!({"/home/worker/src/workspace/rewritten/main.cpp": "class/main.cpp"})), fs::canonicalize("tests").unwrap(), "/home/worker/src/workspace", false, true, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                 count += 1;
                 assert!(abs_path.is_absolute());
    ...
    fn test_rewrite_paths_rewrite_path_using_mapping_and_source_directory_and_remove_prefix() {
            let mut result_map: CovResultMap = HashMap::new();
            result_map.insert("C:\\Users\\worker\\src\\workspace\\rewritten\\main.cpp".to_string(), empty_result!());
            let results = rewrite_paths(result_map, Some(json!({"C:/Users/worker/src/workspace/rewritten/main.cpp": "class/main.cpp"})), fs::canonicalize("tests").unwrap(), "C:\\Users\\worker\\src\\workspace", false, true, Vec::new(), None);
            let mut count = 0;
            for (abs_path, rel_path, result) in results {
                count += 1;
                assert!(abs_path.is_absolute());
    ...
    fn test_rewrite_paths_only_covered() {
        ...
        let results = rewrite_paths(result_map, None, PathBuf::from(""), "", false, false, Vec::new(), Some(true));
        let mut count = 0;
        for (abs_path, rel_path, result) in results {
            count += 1;
    ...
    fn test_rewrite_paths_only_uncovered() {
        ...
        result_map.insert("covered.cpp".to_string(), covered_result!());
        result_map.insert("uncovered.cpp".to_string(), uncovered_result!());
        let results = rewrite_paths(result_map, None, PathBuf::from(""), "", false, false, Vec::new(), Some(false));
        let mut count = 0;
        for (abs_path, rel_path, result) in results {
            count += 1;
