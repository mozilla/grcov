...
impl ParallelWalker {
    ...
    where
        F: Fn(&DirEntry) -> bool + Sync + Send,
    {
        let follow_links = self.follow_links;
        let max_depth = self.max_depth;

        // Create a thread pool with a specific number of threads and names
        let pool = rayon::ThreadPoolBuilder::new()
            .num_threads(num_cpus::get() - 1)
            .build()
            .unwrap();

        // Execute a task within the thread pool and collect results
        pool.install(|| {
            // Process all root paths in parallel and collect results
            if self.paths.len() == 1 {
                // fast path
                Self::walk_dir_parallel_collect(&self.paths[0], 0, &filter, follow_links, max_depth)
            } else {
                self.paths
                    .into_par_iter()
                    .flat_map(|path| {
                        Self::walk_dir_parallel_collect(&path, 0, &filter, follow_links, max_depth)
                    })
                    .collect()
            }
        })
        ...
        F: Fn(&DirEntry) -> bool + Sync + Send,
    {
        ...
        // Use par_bridge() to parallelize the directory iterator efficiently
        entries
            .par_bridge()
            .filter_map(|entry_result| {
                match entry_result {
                    Ok(entry) => {
                        ...
                        let matched = filter(&entry);

                        // Check if it's a directory for recursion
                        let is_dir = file_type.is_ok_and(|file_type| {
                            if file_type.is_dir() {
                                true
                            } else if follow_links && file_type.is_symlink() {
                                // Only check metadata for symlinks if we're following them
                                entry_path
                                    .metadata()
...
mod tests {
    ...
    fn collect_all_entries(walker: ParallelWalker) -> Vec<PathBuf> {
        collect_paths(walker, |_| true)
    }

    fn collect_files(walker: ParallelWalker) -> Vec<PathBuf> {
        collect_paths(walker, |entry| {
    ...
    fn test_directory_filtering() {
        ...
        let files = collect_paths(ParallelWalker::new(dir.path()), |entry| {
            let path = entry.path();
    ...
    fn test_deeply_nested_structure() {
        ...
        for i in 0..20 {
            ...
            create_test_file(&current_path, &format!("file{}.txt", i), b"content");
        }
    ...
    fn test_large_directory() {
        ...
        for i in 0..100 {
            create_test_file(dir.path(), &format!("file{}.txt", i), b"content");

            let subdir = dir.path().join(format!("dir{}", i));
            fs::create_dir_all(&subdir).unwrap();
            create_test_file(&subdir, &format!("subfile{}.txt", i), b"subcontent");
        }
    ...
    fn test_parallel_complex_file_structure() {
        ...
        for i in 0..10 {
            ...
            for j in 0..10 {
                create_test_file(&subdir, &format!("file{}_{}.txt", i, j), b"content");
            }
        }
